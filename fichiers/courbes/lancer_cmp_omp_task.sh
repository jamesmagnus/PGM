...
for SIZE in $SIZES; do for nb in $ITE ; do echo -n "$OUTPUT $SIZE " >> ALL_omp_task ;  $EXE -s $SIZE | tail -n1 >> ALL_omp_task; done; done
}

#for i in 256 512 1024 2048;
#do
execute  -v omp_tile_task
execute  -v omp_tile_task_opt
#done
