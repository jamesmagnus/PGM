...
void ocl_init (void)
{
    ...
    err = clGetPlatformInfo (pf [platform_no], CL_PLATFORM_NAME, 1024, name,
                             NULL);
    ...
    if (nb_devices == 0)
        exit_with_error (
            "No appropriate device found on platform %d (%s - %s). Try PLATFORM=<p> ./prog blabla\n",
            platform_no, name, vendor);
    if (dev >= nb_devices)
    ...
    err = clGetDeviceInfo (devices [dev], CL_DEVICE_TYPE,
    ...
    if (graphics_display_enabled ()) {
        ...
        context = clCreateContext (properties, 1, &devices [dev], NULL, NULL,
                                   &err);
    }
    else {
    ...
    //
    {
        ...
        //
        {
            ...
            if (len > 1) {
                ...
                clGetProgramBuildInfo (program, devices [dev],
...
unsigned ocl_compute (unsigned nb_iter)
{
    ...
    for (unsigned it = 1; it <= nb_iter; it++) {
        ...
        err |=
            clSetKernelArg (compute_kernel, 1, sizeof (cl_mem), &next_buffer);
        err |= clSetKernelArg (compute_kernel, 2, sizeof (cl_mem),
                               &change_buffer);
        err |= clSetKernelArg (compute_kernel, 3, sizeof (cl_mem),
                               &next_change_buffer);

        check (err, "Failed to set kernel arguments");

#ifdef __APPLE__
        err = clEnqueueNDRangeKernel (queue, compute_kernel, 2, NULL, global,
                                      local,
                                      0, NULL, NULL);
#else
        err = clEnqueueNDRangeKernel (queue, compute_kernel, 2, NULL, global,
        ...
        {
            cl_mem tmp = change_buffer;
            change_buffer = next_change_buffer;
            next_change_buffer = tmp;
        }
    }
