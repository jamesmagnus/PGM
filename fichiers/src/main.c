...
static void update_refresh_rate (int p)
{
    ...
    if ((( i_refresh_rate == 0) && ( p < 0) ) ||
        (( i_refresh_rate == 7) && ( p > 0) ))
        return;
...
static void usage (int val)
{
    ...
    fprintf (stderr,
             "\t-d\t| --debug-flags <flags>\t: enable debug messages\n");
    fprintf (stderr,
...
static void filter_args (int *argc,
                         char *argv[])
{
    progname = argv[0];
